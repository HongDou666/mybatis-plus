...
`UserMapperTest` 除 BaseMapper CRUD 外，还覆盖 LambdaQueryWrapper / LambdaUpdateWrapper 条件示例（用户名 like + 余额 ge；按用户名更新余额）。另有自定义 SQL 示例：`UserMapper.deductBalance` 在 XML 中写扣减语句（注解写法已注释保留），WHERE 由 Wrapper 的 `customSqlSegment` 注入。
