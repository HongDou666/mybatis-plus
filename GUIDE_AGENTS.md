...
`UserMapperTest` 除 BaseMapper CRUD 外，还覆盖 LambdaQueryWrapper / LambdaUpdateWrapper 条件示例（用户名 like + 余额 ge；按用户名更新余额）。
...
├── src/main/resources/
│   ├── application.yaml
│   └── mapper/UserMapper.xml    # 空 mapper 壳（无自定义 CRUD SQL）
